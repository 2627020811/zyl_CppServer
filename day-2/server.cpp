...
//对socket、bind、listen都用错误处理函数包装起来
int main()
{
    ...
    while(true)
    {
        ...
        ssize_t read_bytes=read(clnt_sockfd,buf,sizeof(buf)); //从客户端读取数据到buf中
        if(read_bytes>0)
        {
            printf("message from client fd %d: %s\n",clnt_sockfd,buf);
            write(clnt_sockfd,buf,sizeof(buf));//将相同数据回写到客户端
        }
        else if(read_bytes==0) //read==0 表示EOF
        {
